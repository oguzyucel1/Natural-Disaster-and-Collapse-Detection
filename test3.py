...
# Connect Yolo Button Function
def connect_yolo():
    # Create buttons for Live and Select Video mode
    button_live.place(relx=0.03, rely=0.1)
    button_select_video.place(relx=0.15, rely=0.1)
...
def start_video(file_path):
    ...
    button_camera.config(text="Stop Video")
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break
...
button_live = Button(root, text="LIVE", command=start_live, **button1_style)
button_select_video = Button(root, text="Video", command=select_video, **button1_style)


# Connect Yolo button
button_connect_yolo = Button(root, text="Connect Yolo", command=connect_yolo, **button_style)
button_connect_yolo.place(relx=0.01, rely=0.01)

# Buttons for Camera and Folder modes
...
# Release camera when the program
