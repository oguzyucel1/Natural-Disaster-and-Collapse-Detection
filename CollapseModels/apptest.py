...
# Tkinter ana pencere
root = tk.Tk()
root.title("YOLO11 Collapse_Detection")
...
def show_webcam():
    ...
    if camera_on:
        ...
        if ret:
            ...
            # Tespit edilen sınıfları konsola yazdır
...
def detect_objects(image):
    ...
    # Tespit edilen sınıfları ve doğruluk oranlarını güncelle ve konsola yazdır
    detected_classes = results[0].boxes.cls.cpu().numpy()
    confidence_scores = results[0].boxes.conf.cpu().numpy()

    print("\nDetected Objects in Uploaded Image:")
...
# Sayaçları sıfırlama fonksiyonu
def reset_counters():
...
button_reset = Button(root, text="Reset Counters", command=reset_counters, font=("Helvetica", 14),
                      bg="#F1C40F", fg="black", activebackground="#F39C12", width=20, height=2)
button_reset.place(relx=0.5, rely=0.85)
